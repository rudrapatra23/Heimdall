...
function setupBotHandlers(bot: Bot) {
  ...
  bot.on('message', async (ctx) => {
    ...
    if (profile) {
      ...
      await ctx.reply(
        '⚠️ Your Telegram account is not linked to a Heimdall account yet.\n\nVisit the web app and click "Connect Telegram".'
      );
    }
  });
}
...
async function handleTelegramLinking(
  ...
  // Prevent linking this Telegram account to a DIFFERENT Heimdall user
  ...
  if (existingLink) {
    await ctx.reply('⚠️ This Telegram account is already linked to a different Heimdall account.');
    return;
  }
  ...
  await ctx.reply(
    '✅ Telegram connected successfully!\n\nYou can now use Heimdall through Telegram. The email workflow is coming soon.'
  );
}
