 ...
 * that's the "don't treat one interaction as a strong preference" rule.
 */
function nextConfidence(currentConfidence: number, alpha: number, weight: number): number {
  ...
  // Move toward 1, same shape as the EMA on value, capped implicitly by asymptote.
  const next = currentConfidence + (1 - currentConfidence) * effectiveAlpha * 0.5;
...
export async function recordInteractionEvent(event: RawInteractionEvent): Promise<PreferenceRow> {
  ...
  if (event.event_type === "explicit_statement") {
    ...
    const updated = await upsertPreference({
      ...
      decay_rate: existing?.decay_rate ?? domainModule.defaultDecayRate,
    });
  ...
  // We still log the event -- it's evidence a conflict may exist -- but the
  // stored value/confidence don't move.
  if (existing?.locked) {
  ...
  if (extracted === null) {
    // e.g. a "suggestion_shown" with no action yet -- log only, don't update.
  ...
  const alpha = existing?.decay_rate ?? domainModule.defaultDecayRate;
  ...
  const newConfidence = nextConfidence(existing?.confidence ?? 0, alpha, weight);
 ...
 * This keeps segmentation opt-in per call rather than exploding rows automatically.
 */
