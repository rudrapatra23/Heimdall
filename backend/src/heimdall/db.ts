...
const sql = postgres(process.env.DATABASE_URL!, {
  max: 10,
  types: {
    // Postgres numeric/decimal (OID 1700) comes back as a string by default
    // to avoid float precision loss. Parse them back to numbers here so confidence,
    // decay_rate, and other numeric columns match their TypeScript types.
    numeric: {
      to: 1700,
      from: [1700],
